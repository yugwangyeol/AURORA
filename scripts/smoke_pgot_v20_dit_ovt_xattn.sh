#!/bin/bash
# Verifies v20 train, in-training eval, save, W&B overlays, standalone eval,
# DiT OVT cross-attn wiring, and cleanup of smoke artifacts.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v20_dit_ovt_xattn}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v20_dit_ovt_xattn_eval}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v20_dit_ovt_xattn}"
export WANDB_DIR="${OUTPUT_DIR}/wandb"
export PYTHONNOUSERSITE=1
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"

echo "===== PGOT v20 DiT OVT-XATTN SMOKE ====="
...
"${PYTHON}" "${PROJECT_ROOT}/train.py" \
    ...
    --pgot_v14_router_depth 1 \
    --pgot_v14_router_mlp_ratio 4 \
    --pgot_dit_ovt_cross_attn_enable True \
    --pgot_dit_ovt_cross_attn_start_block 25 \
    --pgot_dit_ovt_cross_attn_every_n_blocks 1 \
...
grep -q '"pgot_v14_router_depth": 1' "${OUTPUT_DIR}/config.json"
grep -q '"pgot_dit_ovt_cross_attn_enable": true' "${OUTPUT_DIR}/config.json"
grep -q '"pgot_mask_bce_weight": 1.0' "${OUTPUT_DIR}/config.json"
grep -q '"pgot_latent_distill_enable": false' "${OUTPUT_DIR}/config.json"
grep -q "Sparse slot/register cross-attention blocks" "${OUTPUT_DIR}/smoke_train.log"
! grep -q "\\[CaptionSlot\\] Initialised" "${OUTPUT_DIR}/smoke_train.log"
grep -q "dit_ovt_xattn_context_tokens" "${OUTPUT_DIR}/smoke_train.log"
...
echo "===== PGOT v20 STANDALONE THRESHOLD EVAL ====="
