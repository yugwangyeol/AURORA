#!/bin/bash
# Verifies V21 train, in-training eval, save, W&B responsibility/recon table,
# standalone ovt_owner eval, and cleanup of smoke artifacts.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v21_group_grounded}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v21_eval}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v21_group_grounded}"
export WANDB_DIR="${OUTPUT_DIR}/wandb"
export PYTHONNOUSERSITE=1
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"

echo "===== PGOT v21 GROUP-GROUNDED SMOKE ====="
...
"${PYTHON}" "${PROJECT_ROOT}/train.py" \
    ...
    --pgot_v21_ground_weight 1.0 \
    --pgot_v21_ground_final_weight 0.1 \
    --pgot_v21_ground_anneal_steps 10 \
    --pgot_v21_temperature 1.0 \
    --pgot_v21_position_weight 1.0 \
    --pgot_v21_code_dim 0 \
...
grep -q '"pgot_v21_enable": true' "${OUTPUT_DIR}/config.json"
grep -q '"pgot_mask_bce_weight": 0.0' "${OUTPUT_DIR}/config.json"
grep -q '"pgot_v21_ground_weight": 1.0' "${OUTPUT_DIR}/config.json"
grep -q "loss_v21_ground" "${OUTPUT_DIR}/smoke_train.log"
grep -q "v21_ground_entropy" "${OUTPUT_DIR}/smoke_train.log"
...
test -n "${WANDB_TABLE}"
grep -q '"ovt_owner_overlay"' "${WANDB_TABLE}"
grep -q '"our_recon"' "${WANDB_TABLE}"

echo "===== PGOT v21 STANDALONE RESPONSIBILITY EVAL ====="
