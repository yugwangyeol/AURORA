#!/bin/bash
# PGOT v8.4 evaluation using the exact internal-attention maps trained by v8.4.
set -euo pipefail

MODEL_PATH="${MODEL_PATH:-/home/jovyan/PGOT/checkpoints/pgot_main_v8_4_void_internal_outside}"
VAL_JSONL="${VAL_JSONL:-/home/jovyan/PGOT/data/pgot_val.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/outputs/eval_pgot_v8_4_$(basename "${MODEL_PATH}")}"
...
echo "===== PGOT v8.4 exact LLM-attention Eval ====="
echo "Ckpt:     ${MODEL_PATH}"
echo "Out:      ${OUTPUT_DIR}"
echo "GT:       ${GT_SOURCE}"
echo "Readout:  llm_attention"
echo "Merge:    mean"
echo "Void:     background owner"
