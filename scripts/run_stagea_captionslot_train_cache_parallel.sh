...
bash "${SCRIPT_DIR}/run_stagea_captionslot_train_cache.sh" \
...
echo "Stage A CaptionSlot train-cache logs:"
...
if [[ "${status0}" -ne 0 || "${status1}" -ne 0 ]]; then
  echo "Stage A CaptionSlot train-cache shard run failed. Check logs in ${LOG_DIR}" >&2
  exit 1
fi

trap - INT TERM

"${PYTHON_BIN}" "${SCRIPT_DIR}/merge_stagea_captionslot_train_cache_shards.py" \
