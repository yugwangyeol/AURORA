#!/usr/bin/env bash
# Two-step train/save/reload smoke for MLLM-query CODA object-memory mixing.
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PYTHON="${PYTHON:-/home/jovyan/.conda/envs/scale_rae/bin/python}"
SMOKE_ROOT="$(mktemp -d "${PROJECT_ROOT}/.smoke_oneshot_memory8_register32_mllm_contrastive.XXXXXX")"
...
WANDB_RUN_ID="${run_id}" WANDB_NAME=smoke_memory8_register32_mllm_contrastive \
...
case "${SMOKE_ROOT}" in
    "${PROJECT_ROOT}"/.smoke_oneshot_memory8_register32_mllm_contrastive.*)
        rm -rf -- "${SMOKE_ROOT}"
        ;;
    *)
...
trap - EXIT
echo "PASS: CODA Object-8/Register-32 MLLM-query smoke verified; outputs removed."
