...
SMOKE_ROOT="$(mktemp -d "${PROJECT_ROOT}/.smoke_oneshot_memory4_noprior.XXXXXX")"
...
WANDB_RUN_ID="${run_id}" WANDB_NAME=smoke_oneshot_memory4_noprior \
...
MASTER_PORT="${MASTER_PORT:-29657}" \
    bash "${PROJECT_ROOT}/scripts/train_pgot_oneshot_memory4_noprior.sh" \
...
MODEL_PATH="${RUN_ROOT}/train/checkpoint-2" EVAL_ROOT="${RUN_ROOT}/eval" \
...
KID_SUBSETS=2 KID_SUBSET_SIZE=2 AR_MAX_NEW_TOKENS=128 \
    bash "${PROJECT_ROOT}/scripts/eval_pgot_oneshot_memory4_noprior_parallel.sh"
...
if os.environ["WANDB_MODE"] == "online":
    ...
    banned = (
        ...
        "latent_distill", "owner_gradient_scale", "allocation",
        "e9_gru", "e12_centroid", "dit_soft_routing",
    )
...
case "${SMOKE_ROOT}" in
    "${PROJECT_ROOT}/.smoke_oneshot_memory4_noprior."*) ;;
    *) exit 2 ;;
esac
"${PYTHON}" - <<'PY'
import os
...
assert path.name.startswith(".smoke_oneshot_memory4_noprior.")
shutil.rmtree(path)
assert not path.exists()
PY
trap - EXIT
echo "PASS: no-owner-prior training, in-train eval, save/reload, filtered W&B, TF/AR rFID+KID/count verified; smoke artifacts removed."
