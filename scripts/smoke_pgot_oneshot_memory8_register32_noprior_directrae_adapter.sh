#!/usr/bin/env bash
# Two-GPU train/save/reload smoke for Object-8/Register-32 direct RAE + adapter.
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PYTHON="${PYTHON:-/home/jovyan/.conda/envs/scale_rae/bin/python}"
SMOKE_ROOT="$(mktemp -d "${PROJECT_ROOT}/.smoke_oneshot_memory8_register32_directrae_adapter.XXXXXX")"
...
WANDB_RUN_ID="${run_id}" WANDB_NAME=smoke_memory8_register32_directrae_adapter \
...
MASTER_PORT="${MASTER_PORT:-29673}" \
    bash "${PROJECT_ROOT}/scripts/train_pgot_oneshot_memory8_register32_noprior_directrae_adapter.sh" \
...
KID_SUBSETS=2 KID_SUBSET_SIZE=2 \
    bash "${PROJECT_ROOT}/scripts/eval_pgot_oneshot_memory8_register32_noprior_directrae_adapter_tf.sh"
...
KID_SUBSETS=2 KID_SUBSET_SIZE=2 AR_MAX_NEW_TOKENS=128 \
    bash "${PROJECT_ROOT}/scripts/eval_pgot_oneshot_memory8_register32_noprior_directrae_adapter_ar.sh"
...
case "${SMOKE_ROOT}" in
    "${PROJECT_ROOT}"/.smoke_oneshot_memory8_register32_directrae_adapter.*)
        rm -rf -- "${SMOKE_ROOT}"
        ;;
    *)
...
trap - EXIT
echo "PASS: Object-8/Register-32 direct RAE adapter smoke verified; outputs removed."
