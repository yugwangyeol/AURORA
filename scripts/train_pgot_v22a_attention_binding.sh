#!/bin/bash
# =============================================================================
# PGOT v22a: V15.1 + LLM-internal OVT image-attention binding.
#
# Keeps the stable V15.1 path:
#   - register=0, void=1
#   - V14/V15 OVT+void bottleneck reconstruction
#   - route CE off
#   - sigmoid BCE dot-product grounding on
#   - CODA-aligned train crop + trainable mm_projector
#
# Adds:
#   - patch-normalized OVT->image outside loss on selected LLM layers
#   - weak foreground patch->object competition CE from the same attention maps
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v22a_attention_binding}"
...
PGOT_MASK_LLM_PATCH_OUTSIDE_WEIGHT="${PGOT_MASK_LLM_PATCH_OUTSIDE_WEIGHT:-0.1}"
PGOT_MASK_LLM_PATCH_OUTSIDE_LAYERS="${PGOT_MASK_LLM_PATCH_OUTSIDE_LAYERS:-26,27}"
PGOT_MASK_LLM_PATCH_OUTSIDE_TEMPERATURE="${PGOT_MASK_LLM_PATCH_OUTSIDE_TEMPERATURE:-1.0}"
PGOT_MASK_LLM_PATCH_VOID_WEIGHT="${PGOT_MASK_LLM_PATCH_VOID_WEIGHT:-1.0}"
PGOT_MASK_LLM_IMAGE_USE_WEIGHT="${PGOT_MASK_LLM_IMAGE_USE_WEIGHT:-0.0}"
PGOT_MASK_LLM_IMAGE_USE_MARGIN="${PGOT_MASK_LLM_IMAGE_USE_MARGIN:-0.05}"
PGOT_V22_ATTENTION_COMPETITION_WEIGHT="${PGOT_V22_ATTENTION_COMPETITION_WEIGHT:-0.05}"
PGOT_V22_ATTENTION_COMPETITION_LAYERS="${PGOT_V22_ATTENTION_COMPETITION_LAYERS:-26,27}"
PGOT_V22_ATTENTION_COMPETITION_TEMPERATURE="${PGOT_V22_ATTENTION_COMPETITION_TEMPERATURE:-1.0}"
PGOT_V22_ATTENTION_COMPETITION_INCLUDE_VOID="${PGOT_V22_ATTENTION_COMPETITION_INCLUDE_VOID:-False}"
PGOT_V22_ATTENTION_COMPETITION_BG_WEIGHT="${PGOT_V22_ATTENTION_COMPETITION_BG_WEIGHT:-0.25}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v22a_attention_binding}"
...
echo "===== PGOT v22a Attention Binding — B200 x ${NUM_GPUS} ====="
...
echo "LLM attention outside:  w=${PGOT_MASK_LLM_PATCH_OUTSIDE_WEIGHT}, layers=${PGOT_MASK_LLM_PATCH_OUTSIDE_LAYERS}"
echo "V22 attention comp:     w=${PGOT_V22_ATTENTION_COMPETITION_WEIGHT}, layers=${PGOT_V22_ATTENTION_COMPETITION_LAYERS}, void=${PGOT_V22_ATTENTION_COMPETITION_INCLUDE_VOID}"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_mask_llm_patch_outside_weight "${PGOT_MASK_LLM_PATCH_OUTSIDE_WEIGHT}" \
    --pgot_mask_llm_patch_outside_layers "${PGOT_MASK_LLM_PATCH_OUTSIDE_LAYERS}" \
    --pgot_mask_llm_patch_outside_temperature "${PGOT_MASK_LLM_PATCH_OUTSIDE_TEMPERATURE}" \
    --pgot_mask_llm_patch_void_weight "${PGOT_MASK_LLM_PATCH_VOID_WEIGHT}" \
    --pgot_mask_llm_image_use_weight "${PGOT_MASK_LLM_IMAGE_USE_WEIGHT}" \
    --pgot_mask_llm_image_use_margin "${PGOT_MASK_LLM_IMAGE_USE_MARGIN}" \
    ...
    --pgot_v22_attention_competition_weight "${PGOT_V22_ATTENTION_COMPETITION_WEIGHT}" \
    --pgot_v22_attention_competition_layers "${PGOT_V22_ATTENTION_COMPETITION_LAYERS}" \
    --pgot_v22_attention_competition_temperature "${PGOT_V22_ATTENTION_COMPETITION_TEMPERATURE}" \
    --pgot_v22_attention_competition_include_void "${PGOT_V22_ATTENTION_COMPETITION_INCLUDE_VOID}" \
    --pgot_v22_attention_competition_bg_weight "${PGOT_V22_ATTENTION_COMPETITION_BG_WEIGHT}" \
