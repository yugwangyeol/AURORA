#!/bin/bash
# Single-phase stage1: 16 slots/object + DiT body unfrozen + LoRA on LLM attention
#   - Same setup as no-LoRA variant, but adds LoRA(r=16, alpha=32) on q/k/v/o_proj
#   - LoRA LR = 1e-4 (same as DiT — adapters need more headroom than slot params)
#   - Base LLM weights stay frozen; only LoRA adapters update
#
# Run on a single GPU (e.g. CUDA_VISIBLE_DEVICES=1) — pair with the no-LoRA variant
# on a different GPU for parallel comparison. Use a different MASTER_PORT to avoid
# torchrun rendezvous collisions.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/AURORA/checkpoints/aurora_refcoco_singlephase_slot16_lora_stage1_fp32}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-aurora_refcoco_singlephase_slot16_lora_stage1_fp32}"
