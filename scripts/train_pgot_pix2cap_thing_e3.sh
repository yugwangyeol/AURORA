#!/usr/bin/env bash
# E3: clean Pix2Cap things + soft residual registers + internal owner competition.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_pix2cap_thing_e3}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_pix2cap_thing_e3}"
mkdir -p "${OUTPUT_DIR}"

echo "E3 train manifest: ${TRAIN_JSONL}"
echo "E3 val manifest:   ${VAL_JSONL}"
echo "Batch: ${PER_DEVICE_TRAIN_BATCH_SIZE}/GPU x ${NUM_GPUS} GPU x accum ${GRADIENT_ACCUMULATION_STEPS} = ${EFFECTIVE_BATCH}"
echo "E3: hard register mask OFF; all-layer OVT outside + register outside + owner competition ON"
...
"${PYTHON}" -m torch.distributed.run \
    --nproc_per_node="${NUM_GPUS}" --master_port="${MASTER_PORT:-29543}" \
    ...
    --pgot_e3_attention_competition_layers "${E3_COMPETITION_LAYERS:-all}" \
    --pgot_e3_attention_competition_temperature "${E3_COMPETITION_TEMPERATURE:-1.0}" \
    --pgot_e3_attention_competition_bg_weight "${E3_COMPETITION_BG_WEIGHT:-0.25}" \
