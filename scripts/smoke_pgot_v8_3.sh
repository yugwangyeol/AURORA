#!/bin/bash
# =============================================================================
# PGOT v8.3 smoke test.
# Verifies BCE + outside-log train, in-training eval, save, offline W&B scalar
# and dual-overlay table logging, then threshold and spatial standalone eval.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v8_3}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v8_3_eval}"
DIFFUSION_NORM_STATS_PATH="/home/jovyan/data/siglip2_bn_stats.pt"

mkdir -p "${OUTPUT_DIR}" "${EVAL_OUTPUT_DIR}/threshold" "${EVAL_OUTPUT_DIR}/spatial"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v8_3_bce1_outlog1}"
...
echo "===== PGOT v8.3 SMOKE TEST (BS=${PER_DEVICE_TRAIN_BATCH_SIZE}, steps=${MAX_STEPS}) ====="
...
echo "===== PGOT v8.3 SMOKE EVAL (threshold/coco_instance) ====="
...
echo "===== PGOT v8.3 SMOKE EVAL (spatial_trainmatch/coco_instance) ====="
"${PYTHON}" -m pgot.eval.run_eval \
    --model_path "${OUTPUT_DIR}" \
    --val_jsonl "${VAL_JSONL}" \
