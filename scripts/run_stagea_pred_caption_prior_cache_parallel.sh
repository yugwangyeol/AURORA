...
bash "${SCRIPT_DIR}/run_stagea_pred_caption_prior_cache.sh" \
...
if [[ "${status0}" -ne 0 || "${status1}" -ne 0 ]]; then
  echo "Stage A shard run failed. Check logs in ${LOG_DIR}" >&2
  exit 1
fi

trap - INT TERM

"${PYTHON_BIN}" "${SCRIPT_DIR}/merge_stagea_pred_caption_prior_cache_shards.py" \
