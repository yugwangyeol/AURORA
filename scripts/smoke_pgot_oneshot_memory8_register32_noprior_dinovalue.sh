#!/usr/bin/env bash
# Two-step train/save/reload smoke for frozen DINOv2 visual-memory values.
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PYTHON="${PYTHON:-/home/jovyan/.conda/envs/scale_rae/bin/python}"
SMOKE_ROOT="$(mktemp -d "${PROJECT_ROOT}/.smoke_oneshot_memory8_register32_dinovalue.XXXXXX")"
...
WANDB_RUN_ID="${run_id}" WANDB_NAME=smoke_memory8_register32_dinovalue \
...
"${PYTHON}" "${PROJECT_ROOT}/scripts/verify_oneshot_memory4_smoke.py" \
    ...
    --direct-rae-query disabled --rae-query-adapter disabled \
    --contrastive disabled --memory-value-source dinov2 --memory-value-dim 768 \
    --object-memories 8 --register-memories 32
...
"${PYTHON}" "${PROJECT_ROOT}/scripts/verify_oneshot_memory4_smoke.py" \
    ...
    --direct-rae-query disabled --rae-query-adapter disabled \
    --contrastive disabled --memory-value-source dinov2 --memory-value-dim 768 \
    --object-memories 8 --register-memories 32 --expect-kid

case "${SMOKE_ROOT}" in
    "${PROJECT_ROOT}"/.smoke_oneshot_memory8_register32_dinovalue.*)
        rm -rf -- "${SMOKE_ROOT}"
        ;;
    *)
...
trap - EXIT
echo "PASS: Object-8/Register-32 DINO-value smoke verified; outputs removed."
