#!/bin/bash
# =============================================================================
# PGOT v8.5: register-free, one always-present void token, and LLM-internal
# image-patch-softmax outside supervision with a weak full-attention image-use
# hinge.
#
# Loss = 1.0 LM + 1.5 recon + 1.0 patch-outside + 0.1 image-use.
# No final-hidden dot-product mask loss, BCE, coverage, or competition CE.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v8_5_llm_patch_outside}"
...
PGOT_LLM_PATCH_OUTSIDE_WEIGHT="${PGOT_LLM_PATCH_OUTSIDE_WEIGHT:-1.0}"
PGOT_LLM_IMAGE_USE_WEIGHT="${PGOT_LLM_IMAGE_USE_WEIGHT:-0.1}"
PGOT_LLM_IMAGE_USE_MARGIN="${PGOT_LLM_IMAGE_USE_MARGIN:-0.05}"
PGOT_LLM_PATCH_VOID_WEIGHT="${PGOT_LLM_PATCH_VOID_WEIGHT:-1.0}"
PGOT_LLM_PATCH_TEMPERATURE="${PGOT_LLM_PATCH_TEMPERATURE:-1.0}"
PGOT_LLM_PATCH_LAYERS="${PGOT_LLM_PATCH_LAYERS:-last4}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v8_5_llm_patch_outside}"
...
echo "===== PGOT v8.5 LLM patch-softmax outside - B200 x ${NUM_GPUS} ====="
echo "Output:                 ${OUTPUT_DIR}"
echo "Effective batch:        $((PER_DEVICE_TRAIN_BATCH_SIZE * NUM_GPUS * GRADIENT_ACCUMULATION_STEPS))"
echo "Register / void:        0 / 1"
echo "Loss weights:           LM=${PGOT_LM_LOSS_WEIGHT}, recon=${PGOT_RECON_LOSS_WEIGHT}, patch_outside=${PGOT_LLM_PATCH_OUTSIDE_WEIGHT}, image_use=${PGOT_LLM_IMAGE_USE_WEIGHT}"
echo "Internal attention:     layers=${PGOT_LLM_PATCH_LAYERS}, patch_temp=${PGOT_LLM_PATCH_TEMPERATURE}, image_margin=${PGOT_LLM_IMAGE_USE_MARGIN}, void_component=${PGOT_LLM_PATCH_VOID_WEIGHT}"
echo "Other mask losses:      all off"
echo "Train/eval score:       post-RoPE image-patch softmax + object mean + void"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_mask_llm_patch_outside_weight "${PGOT_LLM_PATCH_OUTSIDE_WEIGHT}" \
    --pgot_mask_llm_patch_outside_layers "${PGOT_LLM_PATCH_LAYERS}" \
    --pgot_mask_llm_patch_outside_temperature "${PGOT_LLM_PATCH_TEMPERATURE}" \
    --pgot_mask_llm_patch_void_weight "${PGOT_LLM_PATCH_VOID_WEIGHT}" \
    --pgot_mask_llm_image_use_weight "${PGOT_LLM_IMAGE_USE_WEIGHT}" \
    --pgot_mask_llm_image_use_margin "${PGOT_LLM_IMAGE_USE_MARGIN}" \
