#!/bin/bash
# Verifies v12 train, in-training eval, save, W&B recon/OVT-owner overlays,
# and standalone OVT-owner evaluation.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v12}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v12_eval}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v12}"
export WANDB_DIR="${OUTPUT_DIR}/wandb"
export PYTHONNOUSERSITE=1
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"

echo "===== PGOT v12 OVT-UPDATE SMOKE ====="
...
"${PYTHON}" "${PROJECT_ROOT}/train.py" \
    ...
    --pgot_v12_layers 12,16,20,24 \
    --pgot_v12_ovt_temperature 1.0 \
    --pgot_v12_owner_temperature 1.0 \
    --pgot_v12_owner_weight 1.0 \
...
grep -q '"pgot_v12_enable": true' "${OUTPUT_DIR}/config.json"
grep -q '"pgot_v12_layers": "12,16,20,24"' "${OUTPUT_DIR}/config.json"
grep -q "loss_ovt_owner" "${OUTPUT_DIR}/smoke_train.log"
grep -q "ovt_fg_acc" "${OUTPUT_DIR}/smoke_train.log"
grep -q "ovt_bg_acc" "${OUTPUT_DIR}/smoke_train.log"
...
echo "===== PGOT v12 STANDALONE OVT-OWNER EVAL ====="
...
grep -q '"owner_source": "v12_ovt_owner_softmax"' "${EVAL_OUTPUT_DIR}/summary.json"
