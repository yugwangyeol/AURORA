#!/bin/bash
# =============================================================================
# PGOT v8.3 — sigmoid BCE coverage + professor-style outside log penalty.
#
# Loss = 1.0 LM + 1.5 recon + 1.0 BCE + 1.0 spatial outside log.
# BCE supervises object coverage with per-patch sigmoid probabilities. Outside
# log independently applies patch-axis softmax and penalizes only patches outside
# the target GT mask with sum -log(1 - A).
# =============================================================================
set -e
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v8_3_bce1_outlog1}"
...
PGOT_SPATIAL_OUTSIDE_LOG_WEIGHT="${PGOT_SPATIAL_OUTSIDE_LOG_WEIGHT:-1.0}"
PGOT_SPATIAL_OUTSIDE_LOG_TEMPERATURE="${PGOT_SPATIAL_OUTSIDE_LOG_TEMPERATURE:-1.0}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v8_3_bce1_outlog1}"
...
echo "===== PGOT v8.3 BCE + spatial outside log — B200 x ${NUM_GPUS} ====="
echo "Output:                 ${OUTPUT_DIR}"
echo "Effective batch:        $((PER_DEVICE_TRAIN_BATCH_SIZE * NUM_GPUS * GRADIENT_ACCUMULATION_STEPS))"
echo "Mask loss:              BCE=${PGOT_MASK_BCE_WEIGHT}, spatial_outside_log=${PGOT_SPATIAL_OUTSIDE_LOG_WEIGHT}, temp=${PGOT_SPATIAL_OUTSIDE_LOG_TEMPERATURE}"
echo "Other mask losses:      CE/Tversky/final-spatial-mass/LLM-QK outside all off"
echo "Eval readouts:          threshold (primary) + spatial_trainmatch (diagnostic)"
echo "Wandb eval table imgs:  ${PGOT_EVAL_LOG_RECON_IMAGES}"
echo "=================================================================="
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_mask_spatial_outside_log_weight "${PGOT_SPATIAL_OUTSIDE_LOG_WEIGHT}" \
    --pgot_mask_spatial_outside_log_temperature "${PGOT_SPATIAL_OUTSIDE_LOG_TEMPERATURE}" \
