#!/bin/bash
# Experiment B: MAX pooling merge mode, recon_loss_weight=1.5
#   - slot attention masks merged with amax instead of mean (training + eval consistent)
#   - recon_loss_weight raised to 1.5 for FID improvement
#   - MASTER_PORT=29504 (no conflict with other runs)

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/AURORA/checkpoints/aurora_refcoco_singlephase_maxpool_lora_stage1_fp32}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-aurora_refcoco_singlephase_maxpool_lora_stage1_fp32}"
