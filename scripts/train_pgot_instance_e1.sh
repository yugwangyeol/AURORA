#!/bin/bash
# E1: COCO instance-only OVTs + constrained residual background registers.
set -euo pipefail

MODEL_PATH="${MODEL_PATH:-/home/jovyan/data/Scale-RAE-Qwen1.5B_DiT2.4B}"
TRAIN_JSONL="${TRAIN_JSONL:-/home/jovyan/PGOT/data/pgot_coco_instance_train.jsonl}"
VAL_JSONL="${VAL_JSONL:-/home/jovyan/PGOT/data/pgot_coco_instance_val.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_instance_e1}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_instance_e1}"
mkdir -p "${OUTPUT_DIR}"
...
"${PYTHON}" -m torch.distributed.run \
    --nproc_per_node="${NUM_GPUS:-2}" --master_port="${MASTER_PORT:-29541}" \
    ...
    --max_steps "${MAX_STEPS:-6000}" --num_train_epochs 100 \
    ...
    --save_strategy steps --save_steps "${SAVE_STEPS:-500}" --save_total_limit 3 \
    ...
    --logging_steps "${LOGGING_STEPS:-10}" --report_to wandb \
