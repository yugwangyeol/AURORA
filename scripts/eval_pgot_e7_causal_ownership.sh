#!/usr/bin/env bash
# Teacher-forced E7 owner/write segmentation + optional 512px reconstruction/rFID.
set -euo pipefail

MODEL_PATH="${MODEL_PATH:-/home/jovyan/PGOT/checkpoints/pgot_e7_causal_ownership_20k/checkpoint-20000}"
VAL_JSONL="${VAL_JSONL:-/home/jovyan/PGOT/data/pgot_pix2cap_generated_val5k.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/outputs/eval_pgot_e7_causal_ownership_20k_tf_coda512}"
...
# E7's register route is already the predicted owner/write distribution. A
# second LLM register-mask forward would evaluate a different architecture.
EXTRA_ARGS+=(--register_eval_route unrestricted)

"${PYTHON}" -m pgot.eval.run_eval \
    --model_path "${MODEL_PATH}" --val_jsonl "${VAL_JSONL}" --output_dir "${OUTPUT_DIR}" \
    --batch_size "${BATCH_SIZE:-32}" --num_workers "${NUM_WORKERS:-4}" \
    --grid_size 32 --max_caption_tokens 1024 --n_ovt_per_object 1 --max_objects 50 \
    --eval_size 512 --readout e7_owner --eval_merge mean \
    --gt_source coco_instance --coco_mask_cache "${COCO_MASK_CACHE}" \
    --image_preprocess_mode coda_center_crop --coda_crop_size 512 \
    --dtype "${DTYPE:-bf16}" --guidance_scale "${GUIDANCE_SCALE:-2.5}" \
