#!/bin/bash
# Verifies v22a train, in-training eval, save, W&B sigmoid/recon overlays,
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v22a_attention_binding}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v22a_attention_binding_eval}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v22a_attention_binding}"
export WANDB_DIR="${OUTPUT_DIR}/wandb"
export PYTHONNOUSERSITE=1
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"

echo "===== PGOT v22a Attention Binding SMOKE ====="
...
"${PYTHON}" "${PROJECT_ROOT}/train.py" \
    ...
    --pgot_mask_llm_patch_outside_weight 0.1 \
    --pgot_mask_llm_patch_outside_layers "26,27" \
    --pgot_mask_llm_patch_outside_temperature 1.0 \
    --pgot_mask_llm_patch_void_weight 1.0 \
    --pgot_mask_llm_image_use_weight 0.0 \
    ...
    --pgot_v22_attention_competition_weight 0.05 \
    --pgot_v22_attention_competition_layers "26,27" \
    --pgot_v22_attention_competition_temperature 1.0 \
    --pgot_v22_attention_competition_include_void False \
    --pgot_v22_attention_competition_bg_weight 0.25 \
...
grep -q '"pgot_mask_llm_patch_outside_weight": 0.1' "${OUTPUT_DIR}/config.json"
grep -q '"pgot_v22_attention_competition_weight": 0.05' "${OUTPUT_DIR}/config.json"
grep -q '"pgot_unfreeze_mm_projector": true' "${OUTPUT_DIR}/config.json"
grep -q '"image_preprocess_mode": "coda_center_crop"' "${OUTPUT_DIR}/config.json"
grep -q "loss_mask_bce" "${OUTPUT_DIR}/smoke_train.log"
grep -q "loss_mask_llm_patch_outside" "${OUTPUT_DIR}/smoke_train.log"
grep -q "loss_v22_attention_competition" "${OUTPUT_DIR}/smoke_train.log"
...
echo "===== PGOT v22a STANDALONE THRESHOLD EVAL ====="
...
echo "V22a smoke complete; smoke artifacts removed."
