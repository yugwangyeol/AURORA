#!/usr/bin/env bash
# E2-Pix-FVW: Pix2Cap E2 hard registers + own-caption OVTs + forced visual writes.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_e2_pix_fvw}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_e2_pix_fvw}"
mkdir -p "${OUTPUT_DIR}"

echo "E2-Pix-FVW initialization: ${MODEL_PATH}"
echo "Train manifest: ${TRAIN_JSONL}"
echo "Val manifest:   ${VAL_JSONL}"
echo "Batch: ${PER_DEVICE_TRAIN_BATCH_SIZE}/GPU x ${NUM_GPUS} GPU x accum ${GRADIENT_ACCUMULATION_STEPS} = ${EFFECTIVE_BATCH}"
echo "Visual write: shared ${FVW_NUM_HEADS:-8}-head image-only overwrite after layers ${FVW_LAYERS:-0,8,16,24,27}"
echo "Role routes: full=${FVW_FULL_PROBABILITY:-0.50}, OVT-only FG=${FVW_OVT_ONLY_PROBABILITY:-0.25}, register-only BG=remainder"
echo "Loss: LM + role-normalized recon + all-layer outside + FVW write-outside; competition/full-inside/tail/counterfactual OFF"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_fvw_enable True --pgot_fvw_layers "${FVW_LAYERS:-0,8,16,24,27}" \
    --pgot_fvw_num_heads "${FVW_NUM_HEADS:-8}" \
    --pgot_fvw_temperature "${FVW_TEMPERATURE:-1.0}" \
    --pgot_fvw_write_outside_weight "${FVW_WRITE_OUTSIDE_WEIGHT:-1.0}" \
    --pgot_fvw_full_probability "${FVW_FULL_PROBABILITY:-0.50}" \
    --pgot_fvw_ovt_only_probability "${FVW_OVT_ONLY_PROBABILITY:-0.25}" \
    --pgot_e5_forcing_probability 0.0 \
    ...
    --pgot_core_outside_temperature 1.0 --pgot_core_void_weight 0.0 \
    --pgot_core_tail_weight 0.0 --pgot_core_tail_fraction 0.0 \
    --pgot_e3_attention_competition_weight 0.0 \
    --pgot_e4_rae_isolated True --pgot_e4_full_inside_weight 0.0 \
    --pgot_e4_rae_bind_weight 0.0 \
