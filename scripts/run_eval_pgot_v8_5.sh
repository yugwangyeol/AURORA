#!/bin/bash
# PGOT v8.5 evaluation using the image-patch-softmax maps trained by v8.5.
set -euo pipefail

MODEL_PATH="${MODEL_PATH:-/home/jovyan/PGOT/checkpoints/pgot_main_v8_5_llm_patch_outside}"
VAL_JSONL="${VAL_JSONL:-/home/jovyan/PGOT/data/pgot_val.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/outputs/eval_pgot_v8_5_$(basename "${MODEL_PATH}")}"
...
echo "===== PGOT v8.5 LLM patch-softmax Eval ====="
echo "Ckpt:     ${MODEL_PATH}"
echo "Out:      ${OUTPUT_DIR}"
echo "GT:       ${GT_SOURCE}"
echo "Readout:  llm_attention (image-patch softmax)"
echo "Merge:    mean"
echo "Void:     residual/background owner"
