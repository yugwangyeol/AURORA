...
SMOKE_ROOT="$(mktemp -d "${PROJECT_ROOT}/.smoke_oneshot_ownergrad_feature.XXXXXX")"
BASE_MODEL="${MODEL_PATH:-${PROJECT_ROOT}/checkpoints/pgot_oneshot_memory4_content/checkpoint-5000}"
...
WANDB_RUN_ID="${run_id}" WANDB_NAME=smoke_oneshot_ownergrad_feature \
...
DTYPE=fp32 DIFFUSION_INFERENCE_STEPS=2 COMPUTE_RFID=True AR_MAX_NEW_TOKENS=128 \
    bash "${PROJECT_ROOT}/scripts/eval_pgot_oneshot_ownergrad_feature_parallel.sh"

"${PYTHON}" "${PROJECT_ROOT}/scripts/verify_oneshot_ownergrad_feature_smoke.py" \
    "${SMOKE_ROOT}" --stage eval
...
case "${SMOKE_ROOT}" in
    "${PROJECT_ROOT}/.smoke_oneshot_ownergrad_feature."*) ;;
    *) exit 2 ;;
esac
"${PYTHON}" - <<'PY'
import os
...
assert path.name.startswith(".smoke_oneshot_ownergrad_feature.")
shutil.rmtree(path)
assert not path.exists()
PY
trap - EXIT
echo "PASS: training, in-train eval, save/reload, W&B, TF/AR verified; smoke artifacts removed."
