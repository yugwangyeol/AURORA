#!/usr/bin/env bash
# One-shot semantic owner Reader: q->owner->raw-SigLIP pooled value.
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
export MODEL_PATH="${MODEL_PATH:-${PROJECT_ROOT}/checkpoints/pgot_e11_capacity_dit16/checkpoint-10000}"
export OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/checkpoints/pgot_oneshot_pooled}"
export WANDB_NAME="${WANDB_NAME:-pgot_oneshot_pooled}"

export ONE_SHOT_READER_ENABLE=True
export ONE_SHOT_READOUT_MODE="${ONE_SHOT_READOUT_MODE:-pooled}"
export ONE_SHOT_DETACH_OWNER_ROUTING=True
export E8_UPDATE_MODE=separate_memory
export E10_RAW_VALUE_ENABLE=False
...
export DIT_UNFREEZE_LAST_N="${DIT_UNFREEZE_LAST_N:-16}"
