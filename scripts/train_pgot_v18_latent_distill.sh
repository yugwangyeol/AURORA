#!/bin/bash
# =============================================================================
# PGOT v18: V15 object-causal bottleneck + direct decoder-latent distillation.
#
# Base path is V15:
#   register=0, void=1, V14 bottleneck forward enabled, route CE disabled,
#   sigmoid BCE mask supervision enabled.
#
# New V18 term:
#   condition_hidden -> SigLIP decoder-native latent head, supervised by
#   MSE + cosine + L1 against gt_siglip. Diffusion recon loss is kept.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v18_v15_latent_distill}"
...
PGOT_LATENT_DISTILL_WEIGHT="${PGOT_LATENT_DISTILL_WEIGHT:-1.0}"
...
PGOT_LATENT_DISTILL_L1_WEIGHT="${PGOT_LATENT_DISTILL_L1_WEIGHT:-1.0}"

export WANDB_PROJECT="${WANDB_PROJECT:-PGOT}"
export WANDB_NAME="${WANDB_NAME:-pgot_main_v18_v15_latent_distill}"
...
echo "===== PGOT v18 V15 + latent distill — B200 x ${NUM_GPUS} ====="
...
echo "Grid size:               32"
echo "Loss weights:            LM=${PGOT_LM_LOSS_WEIGHT}, recon=${PGOT_RECON_LOSS_WEIGHT}, BCE=${PGOT_MASK_BCE_WEIGHT}, latent=${PGOT_LATENT_DISTILL_WEIGHT}"
echo "Latent subweights:       mse=${PGOT_LATENT_DISTILL_MSE_WEIGHT}, cos=${PGOT_LATENT_DISTILL_COS_WEIGHT}, l1=${PGOT_LATENT_DISTILL_L1_WEIGHT}"
echo "Route CE:                ${PGOT_V14_ROUTE_WEIGHT}"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_latent_distill_enable True \
    --pgot_latent_distill_weight "${PGOT_LATENT_DISTILL_WEIGHT}" \
    --pgot_latent_distill_mse_weight "${PGOT_LATENT_DISTILL_MSE_WEIGHT}" \
    --pgot_latent_distill_cos_weight "${PGOT_LATENT_DISTILL_COS_WEIGHT}" \
    --pgot_latent_distill_l1_weight "${PGOT_LATENT_DISTILL_L1_WEIGHT}" \
