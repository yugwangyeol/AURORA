#!/bin/bash
# =============================================================================
# PGOT v12: register-free OVT-update blocks inside the LLM.
#
# Loss = 1.0 LM + 1.5 recon + 1.0 OVT-owner CE
# Train/eval both use the same object-vs-void OVT-owner softmax maps.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v12_ovt_update}"
...
PGOT_V12_OWNER_WEIGHT="${PGOT_V12_OWNER_WEIGHT:-1.0}"
PGOT_V12_LAYERS="${PGOT_V12_LAYERS:-12,16,20,24}"
PGOT_V12_OVT_TEMPERATURE="${PGOT_V12_OVT_TEMPERATURE:-${PGOT_V12_SLOT_TEMPERATURE:-1.0}}"
PGOT_V12_OWNER_TEMPERATURE="${PGOT_V12_OWNER_TEMPERATURE:-1.0}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v12_ovt_update}"
...
echo "===== PGOT v12 OVT-update — B200 x ${NUM_GPUS} ====="
echo "Output:                 ${OUTPUT_DIR}"
echo "Effective batch:        $((PER_DEVICE_TRAIN_BATCH_SIZE * NUM_GPUS * GRADIENT_ACCUMULATION_STEPS))"
echo "Register / void:        0 / 1"
echo "Loss weights:           LM=${PGOT_LM_LOSS_WEIGHT}, recon=${PGOT_RECON_LOSS_WEIGHT}, owner=${PGOT_V12_OWNER_WEIGHT}"
echo "OVT layers:             ${PGOT_V12_LAYERS}"
echo "OVT / owner temp:       ${PGOT_V12_OVT_TEMPERATURE} / ${PGOT_V12_OWNER_TEMPERATURE}"
echo "Other mask losses:      all off"
echo "Train/eval readout:     ovt_owner"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_v12_enable True \
    --pgot_v12_layers "${PGOT_V12_LAYERS}" \
    --pgot_v12_ovt_temperature "${PGOT_V12_OVT_TEMPERATURE}" \
    --pgot_v12_owner_temperature "${PGOT_V12_OWNER_TEMPERATURE}" \
    --pgot_v12_owner_weight "${PGOT_V12_OWNER_WEIGHT}" \
