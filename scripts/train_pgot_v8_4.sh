#!/bin/bash
# =============================================================================
# PGOT v8.4 — register-free, one always-present void token, and exact
# LLM-internal attention outside-log supervision.
#
# Loss = 1.0 LM + 1.5 recon + 1.0 exact-LLM-attention outside.
# The outside term contains equally weighted object and void components.
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v8_4_void_internal_outside}"
...
PGOT_LLM_ATTENTION_OUTSIDE_WEIGHT="${PGOT_LLM_ATTENTION_OUTSIDE_WEIGHT:-1.0}"
PGOT_LLM_ATTENTION_VOID_WEIGHT="${PGOT_LLM_ATTENTION_VOID_WEIGHT:-1.0}"
PGOT_LLM_ATTENTION_LAYERS="${PGOT_LLM_ATTENTION_LAYERS:-last4}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v8_4_void_internal_outside}"
...
echo "===== PGOT v8.4 void + exact LLM-attention outside — B200 x ${NUM_GPUS} ====="
echo "Output:                 ${OUTPUT_DIR}"
echo "Effective batch:        $((PER_DEVICE_TRAIN_BATCH_SIZE * NUM_GPUS * GRADIENT_ACCUMULATION_STEPS))"
echo "Register / void:        0 / 1"
echo "Loss weights:           LM=${PGOT_LM_LOSS_WEIGHT}, recon=${PGOT_RECON_LOSS_WEIGHT}, internal_outside=${PGOT_LLM_ATTENTION_OUTSIDE_WEIGHT}"
echo "Internal outside:       layers=${PGOT_LLM_ATTENTION_LAYERS}, void_component=${PGOT_LLM_ATTENTION_VOID_WEIGHT}"
echo "Other mask losses:      all off"
echo "Train/eval score:       exact selected-layer LLM attention + object mean + void"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_mask_llm_attention_outside_weight "${PGOT_LLM_ATTENTION_OUTSIDE_WEIGHT}" \
    --pgot_mask_llm_attention_outside_layers "${PGOT_LLM_ATTENTION_LAYERS}" \
    --pgot_mask_llm_attention_void_weight "${PGOT_LLM_ATTENTION_VOID_WEIGHT}" \
