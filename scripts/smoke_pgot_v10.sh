...
# threshold eval, and competition eval.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v10}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v10_eval}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v10_bce_ce01}"
...
echo "===== PGOT v10 SMOKE TEST (BS=${PER_DEVICE_TRAIN_BATCH_SIZE}, steps=${MAX_STEPS}) ====="
...
echo "===== PGOT v10 SMOKE EVAL (threshold/coco_instance) ====="
"${PYTHON}" -m pgot.eval.run_eval \
    ...
    --eval_size 224 --readout threshold --gt_source coco_instance --dtype fp32 \
    2>&1 | tee "${EVAL_OUTPUT_DIR}/threshold/smoke_eval.log"

echo "===== PGOT v10 SMOKE EVAL (competition/coco_instance) ====="
"${PYTHON}" -m pgot.eval.run_eval \
    --model_path "${OUTPUT_DIR}" \
    --val_jsonl "${VAL_JSONL}" \
    ...
    --batch_size 1 --num_workers 1 --max_samples 2 \
    --grid_size 32 --max_caption_tokens 2048 --n_ovt_per_object 2 --max_objects 50 \
    --eval_size 224 --readout competition --gt_source coco_instance --dtype fp32 \
