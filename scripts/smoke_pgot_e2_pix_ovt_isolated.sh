#!/usr/bin/env bash
# Two-GPU train/eval/save/offline-W&B/standalone-eval smoke for E2-Pix-OVT-Isolated.
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/checkpoints/pgot_smoke_e2_pix_ovt_isolated}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-${PROJECT_ROOT}/outputs/smoke_pgot_e2_pix_ovt_isolated_eval}"
...
export WANDB_NAME=pgot_smoke_e2_pix_ovt_isolated
...
OUTPUT_DIR="${OUTPUT_DIR}" WANDB_NAME=pgot_smoke_e2_pix_ovt_isolated \
bash "${PROJECT_ROOT}/scripts/train_pgot_e2_pix_ovt_isolated.sh" 2>&1 | tee "${OUTPUT_DIR}/smoke_train.log"
...
grep -q '"pgot_e4_rae_isolated": false' "${OUTPUT_DIR}/checkpoint-1/config.json"
grep -q '"pgot_e3_attention_competition_weight": 0.0' "${OUTPUT_DIR}/checkpoint-1/config.json"
grep -q '"pgot_e4_full_inside_weight": 0.0' "${OUTPUT_DIR}/checkpoint-1/config.json"
...
bash "${PROJECT_ROOT}/scripts/eval_pgot_e2_pix_ovt_isolated.sh" 2>&1 | tee "${EVAL_OUTPUT_DIR}/smoke_eval.log"
...
trap - EXIT
echo "PGOT E2-Pix-OVT-Isolated smoke passed; smoke outputs were removed."
