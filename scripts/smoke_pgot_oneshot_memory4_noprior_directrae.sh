#!/usr/bin/env bash
# Two-GPU train/save/reload smoke for direct RAE query, followed by TF then AR.
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PYTHON="${PYTHON:-/home/jovyan/.conda/envs/scale_rae/bin/python}"
SMOKE_ROOT="$(mktemp -d "${PROJECT_ROOT}/.smoke_oneshot_memory4_noprior_directrae.XXXXXX")"
...
WANDB_RUN_ID="${run_id}" WANDB_NAME=smoke_oneshot_memory4_noprior_directrae \
...
MASTER_PORT="${MASTER_PORT:-29661}" \
    bash "${PROJECT_ROOT}/scripts/train_pgot_oneshot_memory4_noprior_directrae.sh" \
...
KID_SUBSETS=2 KID_SUBSET_SIZE=2 \
    bash "${PROJECT_ROOT}/scripts/eval_pgot_oneshot_memory4_noprior_directrae.sh"
...
KID_SUBSETS=2 KID_SUBSET_SIZE=2 AR_MAX_NEW_TOKENS=128 \
    bash "${PROJECT_ROOT}/scripts/eval_pgot_oneshot_memory4_noprior_directrae_ar.sh"

"${PYTHON}" "${PROJECT_ROOT}/scripts/verify_oneshot_memory4_smoke.py" \
    "${SMOKE_ROOT}" content --stage eval --owner-prior disabled \
    --direct-rae-query enabled --expect-kid

case "${SMOKE_ROOT}" in
    "${PROJECT_ROOT}/.smoke_oneshot_memory4_noprior_directrae."*) ;;
    *) exit 2 ;;
esac
"${PYTHON}" - <<'PY'
import os
...
assert path.name.startswith(".smoke_oneshot_memory4_noprior_directrae.")
shutil.rmtree(path)
assert not path.exists()
PY
trap - EXIT
echo "PASS: direct RAE query train/save/reload + TF + AR verified; smoke outputs removed."
