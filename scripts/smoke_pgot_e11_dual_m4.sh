#!/usr/bin/env bash
# Two-GPU FP32 E11 train/in-train-eval/save/offline-W&B/standalone-eval smoke.
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SMOKE_ROOT="$(mktemp -d "${PROJECT_ROOT}/.smoke_e11_m4.XXXXXX")"
OUTPUT_DIR="${SMOKE_ROOT}/checkpoint"
EVAL_OUTPUT_DIR="${SMOKE_ROOT}/eval"
BASE_MODEL="${MODEL_PATH:-${PROJECT_ROOT}/checkpoints/pgot_e10_r_raw_value/checkpoint-9000}"
...
preserve_failed_smoke() {
    local status=$?
    if (( status != 0 )); then
        echo "E11 smoke failed; outputs preserved at ${SMOKE_ROOT}" >&2
    fi
    return "${status}"
}
...
export WANDB_NAME=pgot_smoke_e11_dual_m4
...
REPORT_TO=wandb bash "${PROJECT_ROOT}/scripts/train_pgot_e11_dual_m4.sh" \
...
grep -q '"pgot_e11_dual_m4_enable": true' "${OUTPUT_DIR}/checkpoint-1/config.json"
grep -q '"pgot_e11_memories_per_owner": 4' "${OUTPUT_DIR}/checkpoint-1/config.json"
...
grep -q 'e11_dual_m4_enabled' "${OUTPUT_DIR}/smoke_train.log"
grep -q 'e11_memory_utilization_entropy' "${OUTPUT_DIR}/smoke_train.log"
grep -q 'e11_memory_assignment_entropy' "${OUTPUT_DIR}/smoke_train.log"
grep -q '\[PGOT/E11\] deterministically initialized four-memory' "${OUTPUT_DIR}/smoke_train.log"
if grep -q "'e11_memory_assignment_entropy': 1.0" "${OUTPUT_DIR}/smoke_train.log"; then
    echo "E11 memory competition stayed exactly uniform" >&2
    exit 1
fi
...
assert any("pgot_e8_writer.memory_id_embeddings" in key for key in keys)
assert any("pgot_e8_reader.memory_key_embeddings" in key for key in keys)
assert any("pgot_e8_writer.raw_value.weight" in key for key in keys)
print("E11 four-memory Writer/Reader tensors verified")
PY
...
bash "${PROJECT_ROOT}/scripts/eval_pgot_e11_dual_m4.sh" \
...
test ! -e "${SMOKE_ROOT}"
trap - EXIT
echo "PGOT E11 Dual-M4 smoke passed; smoke outputs were removed."
