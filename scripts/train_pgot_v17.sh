#!/bin/bash
# =============================================================================
# PGOT v17: Generative Binding.
#
# Warm-start from V15, remove the V14 router, feed final-layer RAE query hidden
# states directly to DiT, and supervise RAE query -> OVT ownership attention.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v17_generative_binding}"
...
PGOT_V17_OWNERSHIP_WEIGHT="${PGOT_V17_OWNERSHIP_WEIGHT:-0.1}"
PGOT_V17_OWNERSHIP_LAYERS="${PGOT_V17_OWNERSHIP_LAYERS:-last4}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v17_generative_binding}"
...
echo "===== PGOT v17 Generative Binding — B200 x ${NUM_GPUS} ====="
...
echo "Grid size:              32"
echo "Loss weights:           LM=${PGOT_LM_LOSS_WEIGHT}, recon=${PGOT_RECON_LOSS_WEIGHT}, BCE=${PGOT_MASK_BCE_WEIGHT}, ownership=${PGOT_V17_OWNERSHIP_WEIGHT}"
echo "Ownership layers:       ${PGOT_V17_OWNERSHIP_LAYERS}"
echo "Condition:              final RAE hidden, router removed"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_v17_ownership_weight "${PGOT_V17_OWNERSHIP_WEIGHT}" \
    --pgot_v17_ownership_layers "${PGOT_V17_OWNERSHIP_LAYERS}" \
