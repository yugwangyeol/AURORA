#!/bin/bash
# =============================================================================
# PGOT v9 — V3-style sigmoid BCE mask supervision + weak spatial outside penalty.
#
# BCE remains the main object-coverage signal. Spatial outside is a regularizer:
# it penalizes each OVT's spatial-softmax attention mass on other annotated
# regions, without replacing the V3 object mask objective.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v9_bce_spatial_out03}"
...
PGOT_SPATIAL_OUTSIDE_WEIGHT="${PGOT_SPATIAL_OUTSIDE_WEIGHT:-0.3}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v9_bce_spatial_out03}"
...
echo "===== PGOT v9 BCE + weak spatial outside — B200 x ${NUM_GPUS} ====="
echo "Output:          ${OUTPUT_DIR}"
echo "Effective batch: $((PER_DEVICE_TRAIN_BATCH_SIZE * NUM_GPUS * GRADIENT_ACCUMULATION_STEPS))"
echo "Mask loss:       BCE=${PGOT_MASK_BCE_WEIGHT}, spatial_outside=${PGOT_SPATIAL_OUTSIDE_WEIGHT}, spatial_temp=${PGOT_SPATIAL_TEMPERATURE}"
echo "Eval baseline:   coco_instance + threshold readout"
echo "=================================================================="
