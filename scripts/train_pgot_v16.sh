#!/bin/bash
# =============================================================================
# PGOT v16: V15 sigmoid BCE masks + V14 bottleneck reconstruction with extra
# recon-only void/scene capacity.
#
# Compared with V15, object OVT mask supervision/readout is unchanged, route CE
# remains disabled, and pgot_n_null_bg is increased so the reconstruction
# bottleneck has additional scene/void carrier tokens.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v16_bce_bottleneck_void${PGOT_N_NULL_BG}}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v16_bce_bottleneck_void${PGOT_N_NULL_BG}}"
...
echo "===== PGOT v16 BCE + OVT bottleneck + recon-only void capacity — B200 x ${NUM_GPUS} ====="
echo "Output:                 ${OUTPUT_DIR}"
echo "Effective batch:        $((PER_DEVICE_TRAIN_BATCH_SIZE * NUM_GPUS * GRADIENT_ACCUMULATION_STEPS))"
echo "Register / void:        0 / ${PGOT_N_NULL_BG}"
echo "Grid size:              32"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_n_register 0 --pgot_n_null_bg "${PGOT_N_NULL_BG}" --pgot_n_ovt_per_object 2 --pgot_max_objects 50 \
