#!/usr/bin/env bash
# E2-Pix-OVT-Isolated: Pix2Cap things + E2 hard registers + isolated OVT rows.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_e2_pix_ovt_isolated}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_e2_pix_ovt_isolated}"
mkdir -p "${OUTPUT_DIR}"

echo "Initialization: ${MODEL_PATH}"
...
echo "OVT: own-caption layer-0 initialization; then image patches + self only in every LLM layer"
echo "Register: GT-union foreground hard block in train; unrestricted and GT-free by default in eval"
echo "RAE: E2 causal 256-query path; sees OVTs + registers + preceding RAE queries"
...
"${PYTHON}" -m torch.distributed.run \
    --nproc_per_node="${NUM_GPUS}" --master_port="${MASTER_PORT:-29545}" \
    ...
    --pgot_ovt_isolated_attention True \
