...
# =============================================================================
# PGOT v11 smoke test.
# Verifies train, in-training eval, save, offline W&B scalar/table logging,
# threshold/mean eval, and competition/mean eval.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v11}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v11_eval}"
DIFFUSION_NORM_STATS_PATH="${DIFFUSION_NORM_STATS_PATH:-/home/jovyan/data/siglip2_bn_stats.pt}"

mkdir -p "${OUTPUT_DIR}" "${EVAL_OUTPUT_DIR}/threshold" "${EVAL_OUTPUT_DIR}/competition"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v11_bal_bce_ce03_mean}"
...
echo "===== PGOT v11 SMOKE TEST (BS=${PER_DEVICE_TRAIN_BATCH_SIZE}, steps=${MAX_STEPS}) ====="
...
echo "===== PGOT v11 SMOKE EVAL (threshold/mean/coco_instance) ====="
"${PYTHON}" -m pgot.eval.run_eval \
    --model_path "${OUTPUT_DIR}" \
    --val_jsonl "${VAL_JSONL}" \
    --output_dir "${EVAL_OUTPUT_DIR}/threshold" \
    --batch_size 1 --num_workers 1 --max_samples 2 \
    --grid_size 32 --max_caption_tokens 2048 --n_ovt_per_object 2 --max_objects 50 \
    --eval_size 224 --readout threshold --eval_merge mean --gt_source coco_instance --dtype fp32 \
    2>&1 | tee "${EVAL_OUTPUT_DIR}/threshold/smoke_eval.log"

echo "===== PGOT v11 SMOKE EVAL (competition/mean/coco_instance) ====="
"${PYTHON}" -m pgot.eval.run_eval \
    --model_path "${OUTPUT_DIR}" \
    --val_jsonl "${VAL_JSONL}" \
    --output_dir "${EVAL_OUTPUT_DIR}/competition" \
    --batch_size 1 --num_workers 1 --max_samples 2 \
    --grid_size 32 --max_caption_tokens 2048 --n_ovt_per_object 2 --max_objects 50 \
    --eval_size 224 --readout competition --eval_merge mean --gt_source coco_instance --dtype fp32 \
    2>&1 | tee "${EVAL_OUTPUT_DIR}/competition/smoke_eval.log"
