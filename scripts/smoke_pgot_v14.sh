#!/bin/bash
# Verifies v14 train, in-training eval, save, W&B recon/OVT-route overlays,
# and standalone OVT-owner route evaluation.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v14}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v14_eval}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v14}"
export WANDB_DIR="${OUTPUT_DIR}/wandb"
export PYTHONNOUSERSITE=1
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"

echo "===== PGOT v14 OVT-BOTTLENECK SMOKE ====="
...
"${PYTHON}" "${PROJECT_ROOT}/train.py" \
    ...
    --max_caption_tokens 2048 --grid_size 16 --eval_num_images 1 --image_aspect_ratio square \
...
grep -q '"pgot_v14_route_weight": 1.0' "${OUTPUT_DIR}/config.json"
grep -q "loss_v14_route" "${OUTPUT_DIR}/smoke_train.log"
grep -q "v14_route_fg_acc" "${OUTPUT_DIR}/smoke_train.log"
grep -q "v14_route_bg_acc" "${OUTPUT_DIR}/smoke_train.log"
...
echo "===== PGOT v14 STANDALONE OVT-ROUTE EVAL ====="
...
grep -q '"owner_source": "v14_ovt_bottleneck_route"' "${EVAL_OUTPUT_DIR}/summary.json"
