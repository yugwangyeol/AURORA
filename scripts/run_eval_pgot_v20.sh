#!/bin/bash
# PGOT v20 evaluation: V15 threshold readout + DiT OVT cross-attn reconstruction.
set -euo pipefail

MODEL_PATH="${MODEL_PATH:-/home/jovyan/PGOT/checkpoints/pgot_main_v20_dit_ovt_xattn}"
VAL_JSONL="${VAL_JSONL:-/home/jovyan/PGOT/data/pgot_val.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/outputs/eval_pgot_v20_$(basename "${MODEL_PATH}")}"
...
DISABLE_DIT_OVT_XATTN="${DISABLE_DIT_OVT_XATTN:-0}"
...
if [ "${DISABLE_DIT_OVT_XATTN}" = "1" ]; then
    EXTRA_ARGS+=(--disable_dit_ovt_cross_attn)
fi

echo "===== PGOT v20 DiT OVT-XAttn Eval ====="
...
echo "Steps:    ${DIFF_INFER_STEPS}"
echo "XAttnOff: ${DISABLE_DIT_OVT_XATTN}"
echo "======================================="
