#!/bin/bash
# Verifies V8.2-R train, in-training eval, save, W&B overlays, and standalone eval.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v8_2_noreg}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v8_2_noreg_eval}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v8_2_noreg}"
export WANDB_DIR="${OUTPUT_DIR}/wandb"
export PYTHONNOUSERSITE=1
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"

echo "===== PGOT v8.2-R REGISTER-FREE SMOKE ====="
...
"${PYTHON}" "${PROJECT_ROOT}/train.py" \
    ...
    --pgot_mask_spatial_outside_log_weight 1.0 \
    --pgot_mask_spatial_outside_log_temperature 1.0 \
...
grep -q "loss_mask_spatial_outside_log" "${OUTPUT_DIR}/smoke_train.log"
...
test -n "${WANDB_TABLE}"
grep -q '"spatial_overlay"' "${WANDB_TABLE}"
grep -q '"our_recon"' "${WANDB_TABLE}"

echo "===== PGOT v8.2-R STANDALONE EVAL ====="
...
grep -q '"readout": "spatial_trainmatch"' "${EVAL_OUTPUT_DIR}/summary.json"
