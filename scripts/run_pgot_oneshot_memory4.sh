#!/usr/bin/env bash
# Two-GPU training, followed by TF/AR evaluation concurrently on one GPU each.
set -euo pipefail
...
export EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/outputs/eval_pgot_oneshot_memory4_${MEMORY_KEY_MODE}}"
mkdir -p "${EVAL_ROOT}"
...
bash "${PROJECT_ROOT}/scripts/train_pgot_oneshot_memory4.sh" 2>&1 | tee "${EVAL_ROOT}/train.log"
export MODEL_PATH="${OUTPUT_DIR}/checkpoint-${MAX_STEPS}"
if [[ ! -f "${MODEL_PATH}/config.json" ]]; then
    # A custom MAX_STEPS need not coincide with periodic SAVE_STEPS.
