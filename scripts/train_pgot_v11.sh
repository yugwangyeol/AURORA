#!/bin/bash
# =============================================================================
# PGOT v11 — object-balanced BCE + mean-merged object competition CE.
#
# Main readout is threshold with mean merge. The competition CE is kept as a
# clustering regularizer, not as the default eval readout.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v11_bal_bce_ce03_mean}"
...
PGOT_MASK_OBJECT_BALANCED_BCE_WEIGHT="${PGOT_MASK_OBJECT_BALANCED_BCE_WEIGHT:-1.0}"
PGOT_MASK_CE_WEIGHT="${PGOT_MASK_CE_WEIGHT:-0.3}"
PGOT_MASK_CE_TEMPERATURE="${PGOT_MASK_CE_TEMPERATURE:-1.0}"
PGOT_MASK_CE_MERGE="${PGOT_MASK_CE_MERGE:-mean}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v11_bal_bce_ce03_mean}"
...
echo "===== PGOT v11 balanced BCE + competition CE — B200 x ${NUM_GPUS} ====="
echo "Output:          ${OUTPUT_DIR}"
echo "Effective batch: $((PER_DEVICE_TRAIN_BATCH_SIZE * NUM_GPUS * GRADIENT_ACCUMULATION_STEPS))"
echo "Mask loss:       object_balanced_bce=${PGOT_MASK_OBJECT_BALANCED_BCE_WEIGHT}, CE=${PGOT_MASK_CE_WEIGHT}, ce_merge=${PGOT_MASK_CE_MERGE}"
echo "Main eval:       threshold readout + mean merge"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_mask_ce_merge "${PGOT_MASK_CE_MERGE}" \
    --pgot_mask_aux_competition_weight 0.0 \
    --pgot_mask_bce_weight 0.0 \
    --pgot_mask_object_balanced_bce_weight "${PGOT_MASK_OBJECT_BALANCED_BCE_WEIGHT}" \
