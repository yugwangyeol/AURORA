...
SMOKE_ROOT="$(mktemp -d "${PROJECT_ROOT}/.smoke_e11_reader3.XXXXXX")"
...
case "${SMOKE_ROOT}" in
    "${PROJECT_ROOT}/.smoke_e11_reader3."*) ;;
...
preserve_failed_smoke() {
    local status=$?
    if (( status != 0 )); then
        echo "E11 Reader-3 smoke failed; outputs preserved at ${SMOKE_ROOT}" >&2
    fi
    return "${status}"
}
...
export WANDB_NAME=pgot_smoke_e11_capacity_dit16_reader3
...
REPORT_TO=wandb bash "${PROJECT_ROOT}/scripts/train_pgot_e11_capacity_dit16_reader3.sh" \
...
grep -q '\[PGOT/Reader\].*source=1 target=3' "${TRAIN_OUTPUT_DIR}/smoke_train.log"
grep -q 'reader_layers=3' "${TRAIN_OUTPUT_DIR}/smoke_train.log"
grep -q 'loss_e8_owner' "${TRAIN_OUTPUT_DIR}/smoke_train.log"
grep -q 'loss_e8_reader' "${TRAIN_OUTPUT_DIR}/smoke_train.log"
grep -q 'e8_reader_num_layers' "${TRAIN_OUTPUT_DIR}/smoke_train.log"
...
assert config["pgot_e8_reader_num_layers"] == 3
...
required = [
    "pgot_e8_reader.refinement_layers.0.output.weight",
    "pgot_e8_reader.refinement_layers.1.output.weight",
]
for name in required:
    assert name in weight_map, name
    shard = os.path.join(root, weight_map[name])
    with safe_open(shard, framework="pt", device="cpu") as handle:
        tensor = handle.get_tensor(name)
    assert torch.isfinite(tensor).all(), name
print("E11 Capacity DiT-16 Reader-3 checkpoint configuration verified")
PY
...
bash "${PROJECT_ROOT}/scripts/eval_pgot_e11_capacity_dit16_reader3.sh" \
    2>&1 | tee "${EVAL_OUTPUT_DIR}/smoke_eval.log"

test -f "${EVAL_OUTPUT_DIR}/summary.json"
grep -q '"e8_reader_num_layers": 3' "${EVAL_OUTPUT_DIR}/summary.json"
...
test ! -e "${SMOKE_ROOT}"
trap - EXIT
echo "PGOT E11 Capacity DiT-16 Reader-3 smoke passed; smoke outputs were removed."
