#!/bin/bash
# =============================================================================
# PGOT v8.2-R — controlled V8.2 ablation with the register path removed.
#
# The only architectural change from V8.2 is pgot_n_register: 64 -> 0.
# Loss = 1.0 LM + 1.5 recon + 1.0 final-dot-product spatial outside log.
# No void token, BCE, competition CE, or LLM-internal attention loss is used.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v8_2_noreg}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v8_2_noreg}"
...
echo "===== PGOT v8.2-R register-free outside-log — B200 x ${NUM_GPUS} ====="
echo "Output:                 ${OUTPUT_DIR}"
echo "Effective batch:        $((PER_DEVICE_TRAIN_BATCH_SIZE * NUM_GPUS * GRADIENT_ACCUMULATION_STEPS))"
echo "Register tokens:        0"
echo "Mask loss:              spatial_outside_log=${PGOT_SPATIAL_OUTSIDE_LOG_WEIGHT}, temp=${PGOT_SPATIAL_OUTSIDE_LOG_TEMPERATURE}"
echo "Other mask losses:      all off"
echo "Train/eval readout:     patch spatial softmax + object mean"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_n_register 0 --pgot_n_null_bg 0 --pgot_n_ovt_per_object 2 --pgot_max_objects 50 \
    ...
    --pgot_rae_query_lr "${RAE_QUERY_LR}" --pgot_llm_lr "${LLM_LR}" \
