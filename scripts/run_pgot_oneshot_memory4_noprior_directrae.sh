#!/usr/bin/env bash
# Two-GPU training, then teacher-forced evaluation, then AR evaluation.
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1}"
export MAX_STEPS="${MAX_STEPS:-5000}"
TRAIN_OUTPUT_DIR="${TRAIN_OUTPUT_DIR:-${PROJECT_ROOT}/checkpoints/pgot_oneshot_memory4_noprior_directrae}"
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/outputs/eval_pgot_oneshot_memory4_noprior_directrae}"
mkdir -p "${EVAL_ROOT}"
...
CUDA_VISIBLE_DEVICES="${EVAL_DEVICE}" MODEL_PATH="${MODEL_PATH}" \
