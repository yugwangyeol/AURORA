#!/bin/bash
# Verifies v8.4 train, in-training eval, save, W&B recon/internal-attention
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v8_4}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v8_4_eval}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v8_4}"
export WANDB_DIR="${OUTPUT_DIR}/wandb"
export PYTHONNOUSERSITE=1
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"

echo "===== PGOT v8.4 VOID + EXACT INTERNAL ATTENTION SMOKE ====="
...
"${PYTHON}" "${PROJECT_ROOT}/train.py" \
    ...
    --pgot_mask_llm_attention_outside_weight 1.0 \
    --pgot_mask_llm_attention_outside_layers last4 \
    --pgot_mask_llm_attention_void_weight 1.0 \
...
grep -q '"pgot_mask_llm_attention_outside_weight": 1.0' "${OUTPUT_DIR}/config.json"
grep -q "loss_mask_llm_attention_outside" "${OUTPUT_DIR}/smoke_train.log"
grep -q "llm_attention_void_outside_mass" "${OUTPUT_DIR}/smoke_train.log"
...
test -n "${WANDB_TABLE}"
grep -q '"llm_attention_overlay"' "${WANDB_TABLE}"
grep -q '"our_recon"' "${WANDB_TABLE}"

echo "===== PGOT v8.4 STANDALONE TRAIN-MATCHED EVAL ====="
