#!/bin/bash
# Verifies v18 train, in-training eval, save, W&B sigmoid/diffusion/direct recon
# table logging, standalone eval, direct-latent rFID path, and cleanup.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_smoke_v18_latent_distill}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-/home/jovyan/PGOT/outputs/smoke_pgot_v18_latent_distill_eval}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_smoke_v18_latent_distill}"
export WANDB_DIR="${OUTPUT_DIR}/wandb"
export PYTHONNOUSERSITE=1
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"

echo "===== PGOT v18 LATENT-DISTILL SMOKE ====="
...
"${PYTHON}" "${PROJECT_ROOT}/train.py" \
    ...
    --pgot_latent_distill_weight 1.0 \
    --pgot_latent_distill_mse_weight 1.0 \
    --pgot_latent_distill_cos_weight 1.0 \
    --pgot_latent_distill_l1_weight 1.0 \
...
grep -q '"pgot_latent_distill_enable": true' "${OUTPUT_DIR}/config.json"
grep -q '"pgot_latent_distill_weight": 1.0' "${OUTPUT_DIR}/config.json"
grep -q "loss_mask_bce" "${OUTPUT_DIR}/smoke_train.log"
grep -q "loss_latent_distill" "${OUTPUT_DIR}/smoke_train.log"
...
grep -q '"direct_recon"' "${WANDB_TABLE}"

echo "===== PGOT v18 STANDALONE THRESHOLD + DIRECT-LATENT EVAL ====="
...
grep -q '"recon_source": "direct_latent"' "${EVAL_OUTPUT_DIR}/summary.json"
grep -q '"rFID"' "${EVAL_OUTPUT_DIR}/summary.json"
