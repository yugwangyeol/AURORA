#!/bin/bash
# =============================================================================
# PGOT v14: OVT bottleneck reconstruction.
#
# Loss = 1.0 LM + 1.5 recon + 1.0 object-balanced OVT route CE.
# Registers are removed. One void token absorbs non-annotated residual regions.
# Diffusion condition is built from OVT/void states only; latent queries are
# positional queries for the bottleneck route.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v14_ovt_bottleneck}"
...
PGOT_V14_ROUTE_WEIGHT="${PGOT_V14_ROUTE_WEIGHT:-1.0}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v14_ovt_bottleneck}"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --max_caption_tokens 2048 --grid_size 16 --eval_num_images 64 --image_aspect_ratio square \
