#!/usr/bin/env bash
# Teacher-forced E6 CODA-protocol segmentation + optional reconstruction/rFID.
set -euo pipefail

MODEL_PATH="${MODEL_PATH:-/home/jovyan/PGOT/checkpoints/pgot_e6_coda_direct/checkpoint-10000}"
VAL_JSONL="${VAL_JSONL:-/home/jovyan/PGOT/data/pgot_pix2cap_generated_val5k.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/outputs/eval_pgot_e6_coda_direct_10k_tf_coda512}"
...
"${PYTHON}" -m pgot.eval.run_eval \
    ...
    --eval_size 512 --readout "${READOUT:-llm_attention}" \
    --llm_attention_readout "${LLM_ATTENTION_READOUT:-core}" --eval_merge mean \
