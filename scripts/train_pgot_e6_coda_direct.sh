#!/usr/bin/env bash
# E6: PGOT final OVT/background states -> CODA Stable-Diffusion direct bottleneck.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_e6_coda_direct}"
CODA_UNET="${CODA_UNET:-/home/jovyan/coda/pretrained/coda/coco/unet/diffusion_pytorch_model.safetensors}"
PYTHON="${PYTHON:-/home/jovyan/.conda/envs/scale_rae/bin/python}"
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

# One B200. Global batch remains 32 for comparison with E2-E5.
NUM_GPUS="${NUM_GPUS:-1}"
...
for path in "${MODEL_PATH}" "${TRAIN_JSONL}" "${VAL_JSONL}" "${CODA_UNET}"; do
...
export WANDB_NAME="${WANDB_NAME:-pgot_e6_coda_direct}"
mkdir -p "${OUTPUT_DIR}"

echo "E6 initialization: ${MODEL_PATH}"
...
echo "Decoder condition: final valid OVTs + 4 background registers only"
echo "Scale-RAE query/DiT path: removed"
...
"${PYTHON}" -m torch.distributed.run \
    --nproc_per_node="${NUM_GPUS}" --master_port="${MASTER_PORT:-29546}" \
    ...
    --pgot_e6_enable True \
    --pgot_e6_model_id stable-diffusion-v1-5/stable-diffusion-v1-5 \
    --pgot_e6_coda_unet_checkpoint "${CODA_UNET}" \
    --pgot_e6_context_dim 768 --pgot_e6_cfg_drop_rate 0.1 \
    --pgot_e6_attention_grid_size 32 \
    ...
    --pgot_e6_projector_lr "${E6_PROJECTOR_LR:-5e-5}" \
    --pgot_e6_unet_lr "${E6_UNET_LR:-1e-5}" \
    ...
    --save_strategy steps --save_steps "${SAVE_STEPS:-5000}" --save_total_limit "${SAVE_TOTAL_LIMIT:-2}" \
    ...
    --pgot_e6_eval_inference_steps "${E6_EVAL_INFERENCE_STEPS:-4}" \
    --pgot_e6_eval_guidance_scale "${E6_EVAL_GUIDANCE_SCALE:-2.5}" \
    --pgot_log_metric_profile e6 \
