#!/usr/bin/env python
"""Fast, minimal CaptionSlot eval — computes rFID / SSIM / PSNR / MSE only.

Per-GPU shard runner. Launch two in parallel (different CUDA_VISIBLE_DEVICES)
and merge with `--action merge` afterwards. No loss reporting, no slot
attention, no image saving — one dataloader pass, one LLM forward per batch.
"""
...
for _p in (str(REPO_ROOT), str(SCALE_RAE_ROOT), str(SCALE_RAE_ROOT / "inference")):
...
DEFAULT_OUTPUT_DIR = "/home/jovyan/AURORA/outputs/captionslot_headprior_s1.0_stage1_eval_fast"
...
def configure_cuda_fp32(allow_tf32: bool) -> None:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--action", choices=["eval", "merge"], default="eval",
                   help="eval: run one shard. merge: combine shard outputs.")
    ...
    p.add_argument("--diffusion-steps", type=int, default=20,
                   help="RF sampler steps (default 20; model default 50).")
    p.add_argument("--allow-tf32", type=str2bool, default=True,
                   help="Keep weights/activations in fp32 while allowing TF32 GEMMs for faster B200 eval.")
    p.add_argument("--torch-compile", type=str2bool, default=False,
                   help="Apply torch.compile to diff_head.model. First batch is slow, later batches are faster.")
    p.add_argument("--save-images", type=int, default=0,
                   help="If >0, save up to this many triptychs this shard "
                        "(input | generated | abs-diff) under <output-dir>/samples/.")
    p.add_argument("--spacy-model", default="en_core_web_sm")
    # merge-only:
    p.add_argument("--shard-dir", dest="shard_dirs", action="append", default=[],
                   help="Shard output directory (repeatable). Used with --action merge.")
    return p.parse_args()
...
def force_cast_model(model, dtype: torch.dtype) -> None:
    """Force-cast the full tree so --dtype actually applies to LLM/DiT/vision."""
    cast_params = 0
    vt_cast = 0
    with torch.no_grad():
        ...
        for b_name, b in model.named_buffers():
            ...
            if dtype != torch.float32 and (
                "normalize_data" in b_name
                or "bn_stats" in b_name
                or b_name.endswith("data_mean")
                or b_name.endswith("data_std")
            ):
                continue
        ...
        for vt in model.get_vision_tower_aux_list():
            ...
            for _, b in vt.named_buffers():
                if b.is_floating_point() and b.dtype != dtype:
                    b.data = b.data.to(dtype=dtype)
    print(
        f"[DTYPE-cast] converted {cast_params} main params + {vt_cast} vision-tower params to {dtype}",
        flush=True,
    )
...
def run_eval(args: argparse.Namespace) -> None:
    ...
    disable_torch_init()
    configure_cuda_fp32(bool(args.allow_tf32))
    ...
    def _dtype_snapshot(tag: str) -> None:
        def _fp(mod):
            try:
    ...
    _dtype_snapshot("pre-cast")
    force_cast_model(model, dtype)
    _dtype_snapshot("post-cast")
    ...
    if getattr(args, "torch_compile", False):
        try:
            if hasattr(model, "diff_head") and hasattr(model.diff_head, "model"):
                print("[INFO] Applying torch.compile to diff_head.model ...", flush=True)
                model.diff_head.model = torch.compile(model.diff_head.model)
                print("[INFO] torch.compile done. First batch will be slow, then faster.", flush=True)
            else:
                print("[WARN] torch_compile requested but diff_head.model was not found; skipping.", flush=True)
        except Exception as exc:
            print(f"[WARN] torch.compile failed: {type(exc).__name__}: {exc}", flush=True)

    if torch.cuda.is_available():
        free, total = torch.cuda.mem_get_info()
        print(
            f"[CUDA] weights-only mem: {(total - free) / 1e9:.1f} GB / {total / 1e9:.1f} GB "
            f"(batch activations will grow this once the first forward starts)",
            flush=True,
        )
    ...
    with torch.inference_mode(), open(per_image_path, "w", encoding="utf-8") as pf:
        pbar = tqdm(loader, desc=f"shard {args.shard_index}/{args.num_shards}",
        ...
        for batch in pbar:
            ...
            for i in range(B):
                rec: Dict[str, Any] = {
                    "image": os.path.abspath(batch["image_path"][i]),
                    "file_name": batch["file_name"][i],
                    "image_id": batch["image_ids"][i],
                }
                try:
                    m = compute_basic_metrics(source[i : i + 1], recon[i : i + 1])
                    rec["metrics"] = {"psnr": m["psnr"], "ssim": m["ssim"], "mse": m["mse"]}
    ...
    # Per-image metric means + (single-shard only) rFID for a self-contained run.
    metrics = _read_metrics(per_image_path)
    if metrics:
        summary["PSNR"] = float(np.mean([m["psnr"] for m in metrics]))
        summary["SSIM"] = float(np.mean([m["ssim"] for m in metrics]))
        summary["MSE"] = float(np.mean([m["mse"] for m in metrics]))
    if args.num_shards == 1 and real_arr.shape[0] >= 2:
        summary["rFID"] = float(compute_fid(real_arr, fake_arr))
...
def _read_metrics(per_image_path: str) -> List[Dict[str, float]]:
    metrics: List[Dict[str, float]] = []
    if not os.path.isfile(per_image_path):
        return metrics
...
def run_merge(args: argparse.Namespace) -> None:
    ...
    merged_per_image = os.path.join(args.output_dir, "per_image.jsonl")
    with open(merged_per_image, "w", encoding="utf-8") as f:
    ...
    merged: Dict[str, Any] = {
        ...
        "allow_tf32": first_summary.get("allow_tf32"),
        "torch_compile": first_summary.get("torch_compile"),
    ...
    if metrics_all:
        merged["PSNR"] = float(np.mean([m["psnr"] for m in metrics_all]))
        merged["SSIM"] = float(np.mean([m["ssim"] for m in metrics_all]))
        merged["MSE"] = float(np.mean([m["mse"] for m in metrics_all]))
    merged["rFID"] = float(compute_fid(real_arr, fake_arr)) if real_arr.shape[0] >= 2 else None
