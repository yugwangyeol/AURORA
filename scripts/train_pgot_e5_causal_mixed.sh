#!/usr/bin/env bash
# E5: E2-Pix hard registers + own-caption OVTs + mixed causal reconstruction.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_e5_causal_mixed}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_e5_causal_mixed}"
mkdir -p "${OUTPUT_DIR}"

echo "E5 initialization: ${MODEL_PATH}"
...
echo "OVT: image + own caption + self; mean-caption layer-0 initialization retained"
echo "Mixed recon: forcing probability=${E5_FORCING_PROBABILITY:-0.25}; forced samples use OVT-only RAE updates + foreground loss"
echo "Loss: LM + mixed recon + all-layer outside + full-inside floor; competition/RAE-bind/tail OFF"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_e5_forcing_probability "${E5_FORCING_PROBABILITY:-0.25}" \
    ...
    --pgot_e4_full_inside_weight "${E5_FULL_INSIDE_WEIGHT:-0.1}" \
    --pgot_e4_full_inside_target "${E5_FULL_INSIDE_TARGET:-0.30}" \
