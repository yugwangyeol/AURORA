#!/bin/bash
# =============================================================================
# PGOT v6a SMOKE TEST (v5 main + aux competition CE on last-layer Q/K)
# Mirrors train_pgot.sh v6, 10 steps, single GPU.
# Verifies: train, in-training eval, save, wandb (offline), recon table, attention overlays.
# =============================================================================
set -e
...
OUTPUT_DIR="/home/jovyan/PGOT/checkpoints/pgot_smoke_v6"
...
# Smoke params (kept tiny for speed; BS overridable for tuning sweeps).
PER_DEVICE_TRAIN_BATCH_SIZE="${PER_DEVICE_TRAIN_BATCH_SIZE:-2}"
MAX_STEPS="${MAX_STEPS:-10}"

echo "===== PGOT v6a SMOKE TEST (v5 + Q/K aux competition)  (BS=${PER_DEVICE_TRAIN_BATCH_SIZE}, steps=${MAX_STEPS}) ====="
...
"${PYTHON}" "${PROJECT_ROOT}/train.py" \
    ...
    \
    --pgot_n_register 64 --pgot_n_ovt_per_object 2 --pgot_max_objects 50 \
    --pgot_lm_loss_weight 1.0 \
    --pgot_mask_ce_weight 1.0 \
    --pgot_mask_ce_temperature 1.0 \
    --pgot_mask_aux_competition_weight 0.5 \
    ...
    --pgot_contrastive_loss_target_weight 0.03 \
    ...
    --max_caption_tokens 2048 --grid_size 32 --eval_num_images 4 --image_aspect_ratio square \
    \
    ...
    --evaluation_strategy steps --eval_steps 5 --prediction_loss_only True \
    --pgot_eval_log_recon_images 2 \
