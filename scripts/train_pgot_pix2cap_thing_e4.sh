#!/usr/bin/env bash
# E4: E3 grounding + full-key OVT visual use + isolated spatial RAE binding.
set -euo pipefail

MODEL_PATH="${MODEL_PATH:-/home/jovyan/PGOT/checkpoints/pgot_pix2cap_thing_e3/checkpoint-10000}"
TRAIN_JSONL="${TRAIN_JSONL:-/home/jovyan/PGOT/data/pgot_pix2cap_thing_train.jsonl}"
VAL_JSONL="${VAL_JSONL:-/home/jovyan/PGOT/data/pgot_pix2cap_thing_val.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_pix2cap_thing_e4}"
PYTHON="${PYTHON:-/home/jovyan/.conda/envs/scale_rae/bin/python}"
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

# One B200: measured E4 peak VRAM is ~132.5 GB at micro-batch 8, while
# micro-batch 16 OOMs. Keep E3's effective global batch 32 via accumulation.
# Override PER_DEVICE_TRAIN_BATCH_SIZE if another GPU is used.
NUM_GPUS="${NUM_GPUS:-1}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_pix2cap_thing_e4}"
mkdir -p "${OUTPUT_DIR}"

echo "E4 initialization: ${MODEL_PATH}"
echo "E4 train manifest: ${TRAIN_JSONL}"
echo "E4 val manifest:   ${VAL_JSONL}"
echo "Batch: ${PER_DEVICE_TRAIN_BATCH_SIZE}/GPU x ${NUM_GPUS} GPU x accum ${GRADIENT_ACCUMULATION_STEPS} = ${EFFECTIVE_BATCH}"
echo "E4: RAE cross-query OFF; OVT full-inside floor + 16x16 RAE owner binding ON"
...
"${PYTHON}" -m torch.distributed.run \
    --nproc_per_node="${NUM_GPUS}" --master_port="${MASTER_PORT:-29544}" \
    ...
    --pgot_register_hard_gt_mask False --pgot_register_hard_gt_mask_eval False \
    ...
    --pgot_core_register_outside_weight "${CORE_REGISTER_OUTSIDE_WEIGHT:-1.0}" \
    --pgot_core_outside_layers all --pgot_core_outside_temperature 1.0 \
    --pgot_core_void_weight 0.0 --pgot_core_tail_weight 0.0 \
    --pgot_e3_attention_competition_weight "${E3_COMPETITION_WEIGHT:-0.1}" \
    --pgot_e3_attention_competition_layers all \
    --pgot_e3_attention_competition_temperature 1.0 \
    --pgot_e3_attention_competition_bg_weight 0.25 \
    --pgot_e4_rae_isolated True \
    --pgot_e4_full_inside_weight "${E4_FULL_INSIDE_WEIGHT:-0.1}" \
    --pgot_e4_full_inside_target "${E4_FULL_INSIDE_TARGET:-0.30}" \
    --pgot_e4_rae_bind_weight "${E4_RAE_BIND_WEIGHT:-0.5}" \
    --pgot_e4_rae_bind_layers "${E4_RAE_BIND_LAYERS:-last8}" \
    --pgot_e4_loss_warmup_steps "${E4_LOSS_WARMUP_STEPS:-500}" \
    ...
    --max_steps "${MAX_STEPS:-5000}" --num_train_epochs 100 \
