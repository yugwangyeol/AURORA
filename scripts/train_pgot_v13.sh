#!/bin/bash
# =============================================================================
# PGOT v13 — V3 sigmoid outside-only BCE + register foreground suppression.
#
# Loss = LM + recon + sigmoid outside(OVT) + register foreground suppression.
# This keeps V3's sigmoid dot-product readout, drops the positive BCE term, and
# prevents registers from carrying annotated object/stuff foreground.
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/jovyan/PGOT/checkpoints/pgot_main_v13_sigmoid_out_reg}"
...
PGOT_SIGMOID_OUTSIDE_WEIGHT="${PGOT_SIGMOID_OUTSIDE_WEIGHT:-1.0}"
PGOT_REGISTER_FOREGROUND_SUPPRESSION_WEIGHT="${PGOT_REGISTER_FOREGROUND_SUPPRESSION_WEIGHT:-0.5}"
...
export WANDB_NAME="${WANDB_NAME:-pgot_main_v13_sigmoid_out_reg}"
...
echo "===== PGOT v13 sigmoid outside + register suppression — B200 x ${NUM_GPUS} ====="
echo "Output:                 ${OUTPUT_DIR}"
echo "Effective batch:        $((PER_DEVICE_TRAIN_BATCH_SIZE * NUM_GPUS * GRADIENT_ACCUMULATION_STEPS))"
echo "Mask loss:              sigmoid_outside=${PGOT_SIGMOID_OUTSIDE_WEIGHT}, register_fg_suppression=${PGOT_REGISTER_FOREGROUND_SUPPRESSION_WEIGHT}"
echo "Other mask losses:      BCE/CE/Tversky/spatial/LLM outside all off"
echo "Eval readout:           threshold + mean merge"
...
"${PYTHON}" -m torch.distributed.run \
    ...
    --pgot_mask_sigmoid_outside_weight "${PGOT_SIGMOID_OUTSIDE_WEIGHT}" \
    --pgot_register_foreground_suppression_weight "${PGOT_REGISTER_FOREGROUND_SUPPRESSION_WEIGHT}" \
