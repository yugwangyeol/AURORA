...
SMOKE_ROOT="$(mktemp -d "${PROJECT_ROOT}/.smoke_oneshot_memory16_register32_noprior.XXXXXX")"
...
WANDB_RUN_ID="${run_id}" WANDB_NAME=smoke_oneshot_memory16_register32_noprior \
...
"${PYTHON}" "${PROJECT_ROOT}/scripts/verify_oneshot_memory4_smoke.py" \
    ...
    --object-memories 16 --register-memories 32 --semantic-registers 4
...
COMMON_EVAL_ENV=(
    ...
    COMPUTE_CLASS_METRICS=True KID_SUBSETS=2 KID_SUBSET_SIZE=2
    AR_MAX_NEW_TOKENS=128
)
...
"${PYTHON}" "${PROJECT_ROOT}/scripts/verify_oneshot_memory4_smoke.py" \
    ...
    --expect-class-metrics --object-memories 16 --register-memories 32 \
...
if os.environ["WANDB_MODE"] == "online":
    ...
    run = api.run(
        f"{entity}/{os.environ['WANDB_PROJECT']}/"
        f"{(root / 'wandb_id.txt').read_text().strip()}"
    )
    ...
    required = (
        ...
        "eval/loss", "eval/loss_recon", "eval/memory_reader_entropy",
        "eval/eval_table",
    )
    for key in required:
    ...
    banned = (
        ...
        "latent_distill", "owner_gradient_scale", "allocation", "e9_gru",
        "e12_centroid", "dit_soft_routing",
    )
...
case "${SMOKE_ROOT}" in
    "${PROJECT_ROOT}/.smoke_oneshot_memory16_register32_noprior."*) ;;
    *) exit 2 ;;
esac
"${PYTHON}" - <<'PY'
import os
...
assert path.name.startswith(".smoke_oneshot_memory16_register32_noprior.")
shutil.rmtree(path)
assert not path.exists()
PY
trap - EXIT
echo "PASS: 16-object/32-register no-prior training, in-train eval, save/reload, filtered W&B, sequential TF/AR rFID+KID/count verified; smoke artifacts removed."
